...
for i in range(6,9):
    ax[i].axvline(x=10,  linestyle='-',linewidth = 5,color='k',alpha=0.2)
for i in range(9):
    ...
    ax2[i].axis([10,25,1,8])
    ax2[i].set_yticks([2,4,6,8])
    ax[i].text(10,0.68,label1[i])
    if i in [0,1,2,3,4,5]:
        ax[i].set_xticklabels([10,15,20,25])
    if i in [1,2,4,5,7,8]:
    ...
    if i in [6,7,8]:
        ...
        ax[i].set_xticklabels([17,22,27,32])
    if i in [2,5,8]:
...
ax[1].text(1,0.8,'Correlation')
ax[1].text(30.5,0.8,'RMSE')
plt.show()
